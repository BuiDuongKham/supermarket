...
public class ExportDetailFunc {
    public final static String BILL_DETAIL_FILE_NAME = "src/main/java/exportdetail.xml";

    private List<Detail> listDetail;
    public ExportDetailFunc() {
    ...
    public List<Detail> readListBillDetails() {
        ...
        if (billDetailXML != null) {
            System.out.println(billDetailXML);
    ...
    public List<Detail> readListBillDetailsByBillId(int billId) {
        ...
        List<Detail> listDetailByBillId = new ArrayList<>();
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i).getBillId() == billId) {
                listDetailByBillId.add(list.get(i));
            }
        }
        return listDetailByBillId;
    }
}
